...
# --- CMS style (like your muLHC script) ---
HAVE_MPLHEP = False
try:
...
def parse_event(block):
    ...
    nup = int(hdr[0])
    idprup = int(hdr[1])
    wgt = float(hdr[2])  # used only for acceptance ratios (weights cancel if constant)
...
class Hist1D:
    def __init__(self, name, bins):
        ...
        # NOTE: counts are now UNWEIGHTED entries (for clean dσ/dx normalization)
...
def book_hists(prefix):
    ...
    add("jet_eta", bins_eta)  # all 4 decay “jets”
    add("jet_pt",  bins_pt1)
...
# -------------------------
# Plotting utilities (CMS-like)
# -------------------------
PLOT_META = {
    "lep_eta":  dict(xlabel=r"$\eta^{\ell}$", ylabel=r"$d\sigma/d\eta^{\ell}$ [pb]", logy=False, xlim=(-6, 6)),
    "lep_pt":   dict(xlabel=r"$p_T^{\ell}$ [GeV]", ylabel=r"$d\sigma/dp_T^{\ell}$ [pb/GeV]", logy=True, xlim=(0, 500)),

    "jet_eta":  dict(xlabel=r"$\eta^{j}$ (4 decay jets)", ylabel=r"$d\sigma/d\eta^{j}$ [pb]", logy=False, xlim=(-6, 6)),
    "jet_pt":   dict(xlabel=r"$p_T^{j}$ [GeV] (4 decay jets)", ylabel=r"$d\sigma/dp_T^{j}$ [pb/GeV]", logy=True, xlim=(0, 500)),

    "b_eta":    dict(xlabel=r"$\eta^{b}$", ylabel=r"$d\sigma/d\eta^{b}$ [pb]", logy=False, xlim=(-6, 6)),
    "b_pt":     dict(xlabel=r"$p_T^{b}$ [GeV]", ylabel=r"$d\sigma/dp_T^{b}$ [pb/GeV]", logy=True, xlim=(0, 500)),

    "lj_eta":   dict(xlabel=r"$\eta^{j_{\mathrm{light}}}$", ylabel=r"$d\sigma/d\eta^{j_{\mathrm{light}}}$ [pb]", logy=False, xlim=(-6, 6)),
    "lj_pt":    dict(xlabel=r"$p_T^{j_{\mathrm{light}}}$ [GeV]", ylabel=r"$d\sigma/dp_T^{j_{\mathrm{light}}}$ [pb/GeV]", logy=True, xlim=(0, 500)),

    "topL_eta": dict(xlabel=r"$\eta(t_{\mathrm{lep}}^{\mathrm{reco}})$", ylabel=r"$d\sigma/d\eta$ [pb]", logy=False, xlim=(-6, 6)),
    "topH_eta": dict(xlabel=r"$\eta(t_{\mathrm{had}}^{\mathrm{reco}})$", ylabel=r"$d\sigma/d\eta$ [pb]", logy=False, xlim=(-6, 6)),

    "topL_pt":  dict(xlabel=r"$p_T(t_{\mathrm{lep}}^{\mathrm{reco}})$ [GeV]", ylabel=r"$d\sigma/dp_T$ [pb/GeV]", logy=True, xlim=(0, 1000)),
    "topH_pt":  dict(xlabel=r"$p_T(t_{\mathrm{had}}^{\mathrm{reco}})$ [GeV]", ylabel=r"$d\sigma/dp_T$ [pb/GeV]", logy=True, xlim=(0, 1000)),

    "topL_m":   dict(xlabel=r"$m(t_{\mathrm{lep}}^{\mathrm{reco}})$ [GeV]", ylabel=r"$d\sigma/dm$ [pb/GeV]", logy=True, xlim=(130, 210)),
    "topH_m":   dict(xlabel=r"$m(t_{\mathrm{had}}^{\mathrm{reco}})$ [GeV]", ylabel=r"$d\sigma/dm$ [pb/GeV]", logy=True, xlim=(130, 210)),

    "Whad_m":   dict(xlabel=r"$m(W_{\mathrm{had}}^{\mathrm{reco}})$ [GeV]", ylabel=r"$d\sigma/dm$ [pb/GeV]", logy=True, xlim=(40, 130)),
}


def hist_to_dsigma(hist: Hist1D, sigma_pb: float):
    edges = hist.bins
    ...
    tot = counts.sum()
    if tot <= 0 or sigma_pb is None:
        return centers, np.zeros_like(centers), widths

    # fraction per bin * sigma / bin_width  => differential cross section
    y = (counts / tot) * (sigma_pb / widths)
    return centers, y, widths


def cms_label(ax, extra_text, right_text):
    if HAVE_MPLHEP:
        try:
            #hep.cms.label("", data=False, ax=ax, loc=0)
            # right-side text (energy etc.)
            #ax.text(0.98, 0.96, right_text, transform=ax.transAxes,
            #        ha="right", va="top", fontsize=14)
            # secondary line under CMS label
            ax.text(0.02, 0.88, extra_text, transform=ax.transAxes,
...
def make_one_plot(outpath_base, var, h_all, h_f5, h_f4, sig_all, sig_f5, sig_f4,
                  title, extra_text, right_text):
    meta = PLOT_META[var]


    fig, ax = plt.subplots(figsize=(10.0, 8.0))

    xA, yA, _ = hist_to_dsigma(h_all[var], sig_all)
    x5, y5, _ = hist_to_dsigma(h_f5[var], sig_f5)
    x4, y4, _ = hist_to_dsigma(h_f4[var], sig_f4)

    # Avoid log(0) issues
    if meta["logy"]:
        yA = np.where(yA > 0, yA, np.nan)
        y5 = np.where(y5 > 0, y5, np.nan)
        y4 = np.where(y4 > 0, y4, np.nan)

    ax.step(xA, yA, where="mid", linewidth=2.7, label="Inclusive (ALL)")
    ax.step(x5, y5, where="mid", linewidth=2.7, label=r"Fiducial: $|\eta_\ell|<5,\ |\eta_j|<5$")
    ax.step(x4, y4, where="mid", linewidth=2.7, label=r"Fiducial: $|\eta_\ell|<4,\ |\eta_j|<4$")

    ax.set_xlabel(meta["xlabel"], fontsize=16)
    ax.set_ylabel(meta["ylabel"], fontsize=16)
    ...
    cms_label(ax, extra_text=extra_text, right_text=right_text)

    fig.tight_layout()
    fig.savefig(outpath_base + ".pdf", dpi=600)
    fig.savefig(outpath_base + ".png", dpi=300)
    plt.close(fig)


def make_all_plots(outdir, tag, h_all, h_f5, h_f4, sig_all, sig_f5, sig_f4):
    plotdir = os.path.join(outdir, "plots")
    os.makedirs(plotdir, exist_ok=True)

    if tag == "LHeC":
        right_text = r"$E_e=50$ GeV, $E_p=7$ TeV"
        title = r"$e^- p \to e^- t\bar{t}$ (semi-leptonic, GEN-level)"
    else:
        right_text = r"$E_{\mu}=500$ GeV, $E_p=7$ TeV"
    ...
    for var in PLOT_META.keys():
        ...
        make_one_plot(outbase, var,
                      h_all=h_all, h_f5=h_f5, h_f4=h_f4,
                      sig_all=sig_all, sig_f5=sig_f5, sig_f4=sig_f4,
                      title=title, extra_text=extra_text, right_text=right_text)
...
def analyze_sample(lhe_path, outdir, tag, max_events=None):
    ...
    for block in iter_event_blocks(lhe_path):
        ...
        # --- histogram fills are UNWEIGHTED for clean dsigma shapes ---
        one = 1.0
    ...
    # --- fiducial cross sections from subprocess xsecs and pass fractions ---
    ...
    # --- NEW: make CMS-style differential plots (PDF+PNG) ---
    make_all_plots(outdir, tag, h_all, h_f5, h_f4,
                   sig_all=xsec_total, sig_f5=sigma_fid5, sig_f4=sigma_fid4)
...
def main():
    ...
    ap.add_argument("--outdir", default="ttbar_genlevel_outputs_noROOT_CMSstyle")
