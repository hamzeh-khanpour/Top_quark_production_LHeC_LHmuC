...
C     
      ...
      DOUBLE PRECISION MUP2_COMPONENTS(N_EE)
...
C         
     ...
     $    EE_COMPONENTS(1:N_EE)
      ...
      IF (ABS(LPP(2)) .GE. 1) THEN
        MUP2=PDG2PDF(LPP(2),-9,2,XBK(2),DSQRT(Q2FACT(2)))
        IF ((ABS(LPP(2)).EQ.4.OR.ABS(LPP(2)).EQ.3)
     $   .AND.PDLABEL.NE.'none') MUP2_COMPONENTS(1:N_EE) =
     $    EE_COMPONENTS(1:N_EE)
      ENDIF
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d~ mu+ > mu+ t t~ d~
      PD(IPROC) = DX1*MUP2
      IF (ABS(LPP(1)).EQ.ABS(LPP(2)).AND. (ABS(LPP(1))
     $ .EQ.3.OR.ABS(LPP(1)).EQ.4).AND.PDLABEL.NE.'none')PD(IPROC)
     $ =EE_COMP_PROD(DX1_COMPONENTS,MUP2_COMPONENTS)
      IPROC=IPROC+1  ! s~ mu+ > mu+ t t~ s~
      PD(IPROC) = SX1*MUP2
      IF (ABS(LPP(1)).EQ.ABS(LPP(2)).AND. (ABS(LPP(1))
     $ .EQ.3.OR.ABS(LPP(1)).EQ.4).AND.PDLABEL.NE.'none')PD(IPROC)
     $ =EE_COMP_PROD(SX1_COMPONENTS,MUP2_COMPONENTS)
      DO I=1,IPROC
