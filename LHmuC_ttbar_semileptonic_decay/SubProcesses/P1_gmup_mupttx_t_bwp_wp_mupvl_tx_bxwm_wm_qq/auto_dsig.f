...
C     and update the couplings accordingly
      ...
      ENDIF
      IF(IPROC.EQ.1) DSIGPROC=DSIG1(P1,WGT,IMODE)  ! g mu+ > mu+ b mu+ vm b~ d u~
      IF(IPROC.EQ.2) DSIGPROC=DSIG2(P1,WGT,IMODE)  ! g mu+ > mu+ b mu+ vm b~ s u~
C     ENDIF
...
C            the warp)
     ...
     $ ,IMIRROR_VEC,VECSIZE_USED)  ! g mu+ > mu+ b mu+ vm b~ d u~
      IF(IPROC.EQ.2) CALL DSIG2_VEC(ALL_P1,ALL_XBK,ALL_Q2FACT
     $ ,ALL_CM_RAP,ALL_WGT,IMODE,ALL_OUT,SYMCONF, CONFSUB,ICONF_VEC
     $ ,IMIRROR_VEC,VECSIZE_USED)  ! g mu+ > mu+ b mu+ vm b~ s u~
